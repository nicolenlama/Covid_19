...
datos_covid<-Covid_19_Victoria[1:62,1:13] # EL N. DE FILAS SE VA INCREMENTANTO UNO DIARIAMENTE
#creo la funci?n armonica para usar la media armonica al estimar los betas por ser ratios
#estas instrucciones de conversion pueden hacer falta
attach(datos_covid)
library(dplyr)
...
Visitas_hospitales<-mean(datos_covid$HOSPITALIZADOS)
visitas_uci<-mean(datos_covid$UCI)
...
as.double(Lambda)
Lambda
lambda_media<-armonica(datos_covid$Lambda)
length(CONTAGIADOS)
